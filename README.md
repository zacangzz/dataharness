...
## How to Run?
1. in Terminal, go to repo root
2. ensure that ```gemma-4-E4B-it-Q4_K_M.gguf``` is available in ```/dist/models```
...
[9 May 2026]
- Using smolagents is somewhat of an overkill and does not fit what I want to do. It's build for multiagent workflows, but when you're working on the local machine, you cannot afford to actually run parallel multi-agent runs, just not enough RAM.
- Typer not great, I moved to use Textual for TUI instead it is more capable and customisable with sufficient reference materials.
- Harness engineering is a complex work of art itself, trying to turn a chaotic LLM into a predictable and deterministic application, I relied on reference materials to determine how best to design this harness and consulted heavily on reference materials in order to come up with the most appropriate 'stack' for this app.

## Personal takeways about Software Development with AI Vibe Coding
...
- I have settled on a workflow using Claude/Codex, where in I get the LLM to record issues and lessons learnt so that they can be reused for the project accordingly. I also got a codemap to help the LLM better identify where to look for specific code.
