"""Knowledge-intent dispatcher (Layer 3).

Maps tool_call names emitted by the runtime in `knowledge` mode (e.g.
`store_workspace_knowledge`) to KnowledgeManager memory-update proposals.

Memory writes are workspace truth and therefore harness-owned (spec §6.13,
§3.4). The dispatcher keeps the mapping in one place rather than scattering
it across orchestrator branches.
"""
